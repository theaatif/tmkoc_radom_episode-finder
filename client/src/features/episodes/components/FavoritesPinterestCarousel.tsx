...
interface FavoritesPinterestCarouselProps {
  episodes: Episode[];
...
export function FavoritesPinterestCarousel({
  episodes,
  onSelectEpisode,
  favoritedIds = new Set(),
  onToggleFavorite,
}: FavoritesPinterestCarouselProps) {
  ...
  // Dynamically redistribute items to prevent cards from disappearing on mobile layout
  ...
  return (
    <div className="w-full flex flex-col items-center">
      {/* Grid container with natural height and no clipping */}
        ...
        return (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/45 to-transparent z-10 flex flex-col justify-end p-5 gap-2">
              {/* Genre tag */}
              <span className="text-[10px] font-bold text-brand-cyan tracking-wider uppercase bg-brand-cyan/20 backdrop-blur-md border border-brand-cyan/20 px-2.5 py-1 rounded-full self-start">
                {episode.genre}
              </span>
